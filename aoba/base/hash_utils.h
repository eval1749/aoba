...
#ifndef AOBA_BASE_HASH_UTILS_H_
#define AOBA_BASE_HASH_UTILS_H_
...
struct hash<std::tuple<Members...>> {
  size_t operator()(const std::tuple<Members...>& tuple) const {
    return aoba::internal::HashTuple(tuple);
  }
};
...
struct hash<std::pair<First, Second>> {
  size_t operator()(const std::pair<First, Second>& pair) const {
    return aoba::HashCombine(std::hash<First>{}(pair.first),
